...
\Plainauthor{Federico Rotolo, Xavier Paoletti, Tomasz Burzykowsi, Marc Buyse, Stefan Michiels} %% comma-separated
\Plaintitle{surrosurv: an R Package for the Evaluation of Failure Time Surrogate Endpoints in Individual Patient Data Meta-Analyses of Randomized Clinical Trials} %% without formatting
