...
library('knitr')
opts_chunk$set(prefix.string = 'figs/fig', 
               fig.height = 4, fig.width = 7,
               out.width = '\\textwidth', prompt = TRUE)
options(replace.assign = TRUE, width = 90, prompt = "R> ", digits = 2)
render_sweave()
@
...
source('~/IGR/misc/R/paletteigr.R')
...
<<survCurves, echo=FALSE, fig.env='figure', fig.lp='fig:', fig.cap='Survival curves for overall survival ($T$) and progression-free survival $S$ in the advanced GASTRIC meta-analysis \\citep{GASTRIC13}'>>=
library(survival)
par(mar = c(4, 4, 2, 0)+.1)
# Overall Survival (T)
plot(survfit(Surv(timeT, statusT) ~ trt, data = gastadv), 
     ...
     xmax = 365.25 * 4, col = 1, lty = 1:2, xscale = 365.25, lwd = LWD, yaxs = 'i', bty = 'l')
# Progression-free Survival (S)
lines(survfit(Surv(timeS, statusS) ~ trt, data = gastadv), 
      xmax = 365.25 * 4, lty = 1:2, xscale = 365.25, add=TRUE, col = 2, lwd = LWD)
legend('topright', t(outer(c('Overall Survival (T)', 'Progression-free Survival (S)'), 
                           c('Control arm', 'Chemotherapy arm'), paste, sep = ' - ')),
...
In this example, the computations required
  \Sexpr{round(as.numeric(allSurroRes$runTime))}
  \Sexpr{attr(allSurroRes$runTime, 'units')}
...
<<predictions, echo=FALSE, fig.env='figure', fig.lp='fig:', fig.cap='Predictions for the advanced GASTRIC meta-analysis \\citep{GASTRIC13}, as computed by the adjusted Clayton copula model, which had poor convergence metrics, and by the Poisson TI model, which was deemed to have converged. HR = hazard ratio.'>>=
  plot(allSurroRes, c('Clayton adj', 'PoissonTI'), 
       xlim = c(.35, 1.4), ylim = c(.4, 1.8), mfrow=1:2)
@
<<eval=FALSE>>=
  plot(allSurroRes, c('Clayton adj', 'PoissonTI'))
@
...
<<echo = 1>>=
  ...
  loocvLegend <- paste(
    ...
    # "The symbol \`\\textbf{X}\' means that the surrogacy model could not be fitted due to numerical problems."
...
<<label=poissonize, echo=2:4, fig.env='figure', fig.cap='Overall survival curves in the advanced GASTRIC meta-analysis \\citep{GASTRIC13}. \\textbf{(a)} Comparison between the survival probability obtanied using the Breslow estimator in the Cox model (solid lines) and those obtained using the auxiliary Poisson model (dashed lines). \\textbf{(b)} Piecewise constant hazard estimated by the auxiliary Poisson model', fig.lp='fig:'>>=
  par(mar = c(4, 3, 3, .3) + .1)
...
The treatment effect estimated by the Cox model is
  \Sexpr{coef(fitcox)} (SE = \Sexpr{sqrt(fitcox$var)}), 
  and it is of 
  \Sexpr{coef(fitpoi)['trt']} 
  (SE = \Sexpr{summary(fitpoi)$coefficients['trt', 'Std. Error']})
