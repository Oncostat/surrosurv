...
fitClayton <- function(data, varcor = FALSE, 
  ...
  if (twoStep) {
    fixed <- c(NA,                 # copula dependence parameter
               ...
               rep(NA, nTrials),   # alphas
               log(Mpars$lambdaT), # baseline scales for T
               log(Mpars$rhoT),    # baseline shapes for T
               rep(NA, nTrials))   # betas
  } else {
    fixed <- rep(NA, length(INIpars))
  }
  
  # library('optimx')
  RES <- optimx(INIpars, mloglik,
                FIXpars = fixed,
...
copuSurr <- function(data, family=c('clayton', 'plackett', 'hougaard'), 
  ...
  if (varcor1) RES$adj <- c(RES$adj, list(VarCor1 = step1$VarCor))
