predict.surrosurv <- function(object,
  ...
  if (exact.models) {
    ind <-
      which(tolower(noSpP(names(object))) %in% tolower(noSpP(models)))
  } else {
  ...
  allRES <- c(# Copula models
    ...
    lapply(poissons, function(poi) {
      ...
      as.data.frame(t(t(object[[poi]]$ranef$trialref[, c('trtS', 'trtT')]) +
  ...
  attr(allRES, 'predf') <- c(# Copula models
    sapply(copulas, function(cop) {
      list(unadj = Vectorize(function(x) {
      ...
      adj = Vectorize(function(x) {
        ...
        pVAR <-
          pdVCOV[1, 1] + 2 * x * pdVCOV[1, 2] + x ^ 2 * pdVCOV[2, 2] +
  ...
  attr(allRES, 'surro.stats') <- attr(allRES, 'surro.stats')[noSpP(rownames(attr(allRES, 'surro.stats'))) %in%
...
print.predictSurrosurv <- function(x, n = 6, ...) {
  ...
  for (i in 1:length(x)) {
    ...
    rownames(res2print) <- paste0(sub('trt', '    Treatment effects on ', rownames(res2print)), ':')
...
print.steSurrosurv <- function(x, digits = 2, ...) {
  res2print <- cbind(beta = round(x, digits),
...
plot.predictSurrosurv <- function(x,
  ...
  # ************************************************************************** #
  if (missing(xlab))
    xlab <- 'Treatment effect (HR) on S'
  if (missing(ylab))
    ylab <- 'Treatment effect (HR) on T'
  if (missing(exact.models))
    exact.models <- all(sapply(tolower(noSpP(models)), function(m)
      any(tolower(noSpP(
        names(x)
  ...
  if (length(x)) {
    ...
    for (i in 1:length(x)) {
      ...
      predf.fit <- Vectorize(function(y) {
        PREDF[[i]](y)[1]
      })
      ...
      axis(1,
           at = axTicks(1),
           labels = sapply(exp(axTicks(1)), function(x)
             ifelse(
               x < .1,
               format(x, nsmall = 3, digits = 2),
               ifelse(
                 x < 1,
                 format(x, nsmall = 2, digits = 2),
                 ifelse(
                   x < 10,
                   format(x, nsmall = 1, digits = 2),
                   format(x, digits = 2, drop0trailing =
                            FALSE)
                 )
               )
             )))
      axis(2,
           at = axTicks(2),
           labels = sapply(exp(axTicks(2)), function(x)
             ifelse(
               x < .1,
               format(x, nsmall = 3, digits = 2),
               ifelse(
    ...
    warning('No model selected', immediate. = TRUE)
}